...
public abstract class Resource {
    ...
    protected <R extends Representation, P extends RequestParameters> Response post(
        ...
        prepareParams(params, false);
        prepareData(content, params, true);
    ...
    protected <R extends Representation, P extends RequestParameters> Response put(
        ...
        prepareParams(params, true);
        prepareData(content, params, false);
    ...
    /**
     * Prepares the parameters for the backend call.
     * 
     * @param params
     ...
     *            the injected parameters with the target version
     * @param create
     *            true if the data should be validated for create, false if for
     *            update
     * @throws WebApplicationException
     */
    protected void prepareData(Representation content, RequestParameters params,
        ...
        if (create) {
            content.validateCreate();
        } else {
            content.validateUpdate();
        }

        content.setVersion(params.getVersion());
        content.update();
    }
